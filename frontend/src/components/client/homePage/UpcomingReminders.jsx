export default function UpcomingReminders({
  ...
  const allPendingReminders = sourceReminders.filter((reminder) => !reminder.completed);
  const allCompletedReminders = sourceReminders.filter((reminder) => reminder.completed);
  ...
  const handleShowAll = () => {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      ...
      {hideCompletedReminders && completedReminders.length === 0 && reminders.length > 0 && (
      ...
      {reminders.length === 0 ? (
        <div className="text-center text-gray-400 text-sm py-6">
          {hideCompletedReminders ? "All activities completed and hidden!" : "No upcoming activities for today."}
        </div>
      ) : (
        <div className="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 pr-2">
          <div className="space-y-3">
            {reminders.map((reminder, index) => (
              <div
                key={index}
                className={`p-3 border rounded-lg flex items-center justify-between ${reminder.completed
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4
                      className={`font-medium text-gray-900 text-sm ${reminder.completed
      ...
      {allCompletedReminders.length > 0 && allPendingReminders.length === 0 && !hideCompletedReminders && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <button
      ...
      {hideCompletedReminders && allCompletedReminders.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          ...
          >
            Show All Activities ({allCompletedReminders.length + allPendingReminders.length})
          </button>
        </div>
      )}
    </div>
