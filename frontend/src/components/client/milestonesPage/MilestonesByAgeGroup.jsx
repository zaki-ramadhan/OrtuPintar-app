...
export default function MilestonesByAgeGroup({
    ...
    const childAge = currentChild ? calculateAgeInYears(currentChild.birthDate) : 0;
    ...
    const milestonesByAge = useMemo(() => {
        ...
        milestones.forEach(milestone => {
            ...
            const maxAge = milestone.age_group_max || 5;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <select
                        ...
                        className="text-sm border border-gray-300 rounded-lg px-2 sm:px-3 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
            ...
            {filteredGroups.length === 0 ? (
                ...
                <div className="space-y-4 sm:space-y-6">
                    {filteredGroups.map((group, groupIndex) => {
                        const completedCount = group.milestones.filter(m => m.isCompleted).length;
                        const totalCount = group.milestones.length;
                        const progressPercent = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;
                        const isExpanded = expandedGroup === groupIndex;
                        const isCurrentAge = group.minAge <= childAge + 1 && group.maxAge >= childAge - 1;

                        return (
                            <div
                                key={`${group.minAge}-${group.maxAge}`}
                                className={`border rounded-xl transition-all duration-200 ${isCurrentAge
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div className="w-24 bg-gray-200 rounded-full h-2">
                                                <div
                                                    className={`h-2 rounded-full transition-all duration-300 ${isCurrentAge ? 'bg-blue-500' : 'bg-emerald-500'
                                                        }`}
...
function MilestoneCard({
    ...
    return (
        <div className={`flex items-center justify-between p-3 sm:p-4 rounded-xl border transition-all duration-200 hover:shadow-md group ${milestone.isCompleted
            ...
            <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                    <h5 className="font-semibold text-gray-900 truncate text-sm sm:text-base">
                        {milestone.title}
                    </h5>
                    <div className="flex flex-wrap items-center gap-1 sm:gap-2 mt-1">
                        <span className={`text-xs px-2 py-1 rounded-full border ${getCategoryColor(milestone.category)}`}>
                            {milestone.category}
                        </span>
            ...
            <div className="ml-2 sm:ml-4 flex-shrink-0">
                {milestone.isCompleted ? (
                    ...
                    <button
                        ...
                        className={`px-2 sm:px-4 py-1 sm:py-2 rounded-lg font-medium text-xs sm:text-sm transition-colors ${milestone.isAgeAppropriate || isCurrentAge
